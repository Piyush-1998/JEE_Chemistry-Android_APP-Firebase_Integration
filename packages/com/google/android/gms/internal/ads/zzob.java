package com.google.android.gms.internal.ads;

/* compiled from: com.google.android.gms:play-services-ads@@18.2.0 */
/* loaded from: classes.dex */
public final class zzob {
    private byte[] data;
    private int zzbgg;
    private int zzbgh = 0;
    private int zzbgi;

    public zzob(byte[] bArr, int i, int i2) {
        this.data = bArr;
        this.zzbgg = i;
        this.zzbgi = i2;
        zzil();
    }

    public final void zzbd(int i) {
        int i2 = this.zzbgg;
        int i3 = (i / 8) + i2;
        this.zzbgg = i3;
        int i4 = this.zzbgh + (i % 8);
        this.zzbgh = i4;
        if (i4 > 7) {
            this.zzbgg = i3 + 1;
            this.zzbgh = i4 - 8;
        }
        while (true) {
            i2++;
            if (i2 <= this.zzbgg) {
                if (zzbe(i2)) {
                    this.zzbgg++;
                    i2 += 2;
                }
            } else {
                zzil();
                return;
            }
        }
    }

    public final boolean zzih() {
        return zzbc(1) == 1;
    }

    public final int zzbc(int i) {
        int i2;
        int i3;
        if (i == 0) {
            return 0;
        }
        int i4 = i / 8;
        int i5 = 0;
        for (int i6 = 0; i6 < i4; i6++) {
            int i7 = zzbe(this.zzbgg + 1) ? this.zzbgg + 2 : this.zzbgg + 1;
            int i8 = this.zzbgh;
            if (i8 != 0) {
                byte[] bArr = this.data;
                i3 = ((bArr[i7] & 255) >>> (8 - i8)) | ((bArr[this.zzbgg] & 255) << i8);
            } else {
                i3 = this.data[this.zzbgg];
            }
            i -= 8;
            i5 |= (255 & i3) << i;
            this.zzbgg = i7;
        }
        if (i > 0) {
            int i9 = this.zzbgh + i;
            byte b = (byte) (255 >> (8 - i));
            int i10 = zzbe(this.zzbgg + 1) ? this.zzbgg + 2 : this.zzbgg + 1;
            if (i9 > 8) {
                byte[] bArr2 = this.data;
                i2 = (b & (((255 & bArr2[i10]) >> (16 - i9)) | ((bArr2[this.zzbgg] & 255) << (i9 - 8)))) | i5;
                this.zzbgg = i10;
            } else {
                i2 = (b & ((255 & this.data[this.zzbgg]) >> (8 - i9))) | i5;
                if (i9 == 8) {
                    this.zzbgg = i10;
                }
            }
            i5 = i2;
            this.zzbgh = i9 % 8;
        }
        zzil();
        return i5;
    }

    public final int zzii() {
        return zzik();
    }

    public final int zzij() {
        int zzik = zzik();
        return (zzik % 2 == 0 ? -1 : 1) * ((zzik + 1) / 2);
    }

    private final int zzik() {
        int i = 0;
        while (!zzih()) {
            i++;
        }
        return ((1 << i) - 1) + (i > 0 ? zzbc(i) : 0);
    }

    private final boolean zzbe(int i) {
        if (2 > i || i >= this.zzbgi) {
            return false;
        }
        byte[] bArr = this.data;
        return bArr[i] == 3 && bArr[i + (-2)] == 0 && bArr[i - 1] == 0;
    }

    private final void zzil() {
        int i;
        int i2;
        int i3 = this.zzbgg;
        zznr.checkState(i3 >= 0 && (i = this.zzbgh) >= 0 && i < 8 && (i3 < (i2 = this.zzbgi) || (i3 == i2 && i == 0)));
    }
}
