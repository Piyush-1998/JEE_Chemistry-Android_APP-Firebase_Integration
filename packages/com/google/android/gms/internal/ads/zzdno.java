...
public final class zzdno {
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public static void zza(ECPublicKey eCPublicKey) throws GeneralSecurityException {
    ...
    public static ECParameterSpec zza(zzdnq zzdnqVar) throws NoSuchAlgorithmException {
        int i = zzdnn.zzhdj[zzdnqVar.ordinal()];
        if (i != 1) {
            if (i != 2) {
                if (i == 3) {
                ...
                String valueOf = String.valueOf(zzdnqVar);
    ...
    public static ECPublicKey zza(zzdnq zzdnqVar, byte[] bArr, byte[] bArr2) throws GeneralSecurityException {
        ECParameterSpec zza = zza(zzdnqVar);
        ECPoint eCPoint = new ECPoint(new BigInteger(1, bArr), new BigInteger(1, bArr2));
        zza(eCPoint, zza.getCurve());
        return (ECPublicKey) zzdnu.zzhef.zzhf("EC").generatePublic(new ECPublicKeySpec(eCPoint, zza));
    }

    public static ECPrivateKey zza(zzdnq zzdnqVar, byte[] bArr) throws GeneralSecurityException {
        return (ECPrivateKey) zzdnu.zzhef.zzhf("EC").generatePrivate(new ECPrivateKeySpec(new BigInteger(1, bArr), zza(zzdnqVar)));
    }

    public static KeyPair zza(ECParameterSpec eCParameterSpec) throws GeneralSecurityException {
        KeyPairGenerator zzhf = zzdnu.zzhee.zzhf("EC");
        zzhf.initialize(eCParameterSpec);
        return zzhf.generateKeyPair();
    }
    ...
    public static byte[] zza(ECPrivateKey eCPrivateKey, ECPoint eCPoint) throws GeneralSecurityException {
        ...
        PublicKey generatePublic = zzdnu.zzhef.zzhf("EC").generatePublic(new ECPublicKeySpec(eCPoint, eCPrivateKey.getParams()));
        KeyAgreement zzhf = zzdnu.zzhed.zzhf("ECDH");
        zzhf.init(eCPrivateKey);
        try {
            zzhf.doPhase(generatePublic, true);
            byte[] generateSecret = zzhf.generateSecret();
