...
public abstract class zzdbd<E> extends zzday<E> implements List<E>, RandomAccess {
    ...
    public static <E> zzdbd<E> zzc(Object[] objArr) {
        return zzb(objArr, objArr.length);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public static <E> zzdbd<E> zzb(Object[] objArr, int i) {
        if (i == 0) {
            return (zzdbd<E>) zzdbn.zzgpp;
        }
            ...
            if (size == list.size()) {
                if ((this instanceof RandomAccess) && (list instanceof RandomAccess)) {
                    for (int i = 0; i < size; i++) {
                        if (zzdao.equal(get(i), list.get(i))) {
                        }
                    }
                    return true;
                }
                zzdbd<E> zzdbdVar = this;
                int size2 = zzdbdVar.size();
